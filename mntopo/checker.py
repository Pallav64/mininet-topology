...
class Checker(object):
    ...
    def test(self):
        ...
        while self.loop_max <= 0 or current_loop > 0:
            ...
            # __print__ -> __print()__ in Python 3
    ...
    def _get_nodes_and_links(self, topology_name):
        ...
        if resp is not None and resp.status_code == 200 and resp.content is not None:
            # __resp.content__ -> __resp.content.decode()__ in Python 3
            ...
            if topology is not None and len(topology) > 0:
                ...
                if nodes is not None:
                    for node in nodes:
                        # __unicode()__ replaced with __str()__ in Python 3
                        if str(node['node-id']).startswith('host'):
                            continue
        ...
        print("Topology {} has {} nodes and {} links. {} ".format(topology_name, len(nodelist), len(linklist), list(nodelist.keys())))
        return nodelist, linklist

    def counter(self):
        nodecounter = {}
        # __print__ -> __print()__ in Python 3
        print("Counting all flows and groups ...")
        ...
        for node in data['nodes']['node']:
            # __unicode()__ replaced with __str()__ in Python 3
            nodeid = str(node['id'])
            if str('controller-config') == nodeid:
                continue
    ...
    def put(self):
        ...
        print("sleeping 10 seconds to let the controller configure all groups before sending flows")
        time.sleep(10)
        nodesdir = self.servicesdir + '/nodes'
    ...
    def _is_valid_topology_links(self, running=True, topology_name='flow:1'):
        ...
        if not found_error:
            print("all links have been detected properly for topology {}".format(topology_name))
            return True
        return False
    ...
    def _is_valid_topology_nodes(self, running=True, topology_name='flow:1'):
        ...
        if not found_error:
            print("all nodes have been detected properly for topology {}".format(topology_name))
            return True
        return False
            ...
            for nodeid in ovs_flows_groups_stats:
                ...
                if os.path.exists(filename):
                    with open(filename, 'r') as infile:
                        ...
                        try:
                            for cookie, flow in node['flows'].items():
                            ...
                            for groupid, group in node['groups'].items():
    ...
    def _ask_retry(self):
        if not self.ask_for_retry:
            return False, False
        var = input("Do you want to ignore current error? yes/no:")  # Use input() in Python 3
        print("you entered", var)
        if 'yes' == var:
            return False, True
        var = input("Do you want to retry? yes/no:")  # Use input() in Python 3
        print("you entered", var)
        if 'yes' == var:
            return True, False
        return False, False
