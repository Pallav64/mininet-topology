...
class Topo(object):
    ...
    def start(self):
        ...
        for interface in self.props['interface']:
            name = interface['name']
            print(f"adding interface {name} to switch {interface['switch']}")
    ...
    def get_nodes_flows_groups(self, prefix=None):
        ...
        for name in self.switches_openflow_names:
            ...
            output = subprocess.check_output(f"sudo ovs-ofctl dump-groups {name} --protocol=Openflow13", shell=True).decode('utf-8')  # Decode bytes to string for Python 3
            pattern = r'group_id=(\d+)'
            regex = re.compile(pattern, re.IGNORECASE)
            for match in regex.finditer(output):
                nodes[oname]['groups'].append(int(match.group(1)))

            output = subprocess.check_output(f"sudo ovs-ofctl dump-flows {name} --protocol=Openflow13", shell=True).decode('utf-8')  # Decode bytes to string for Python 3
    ...
    def get_nodes_flows_groups_stats(self, prefix=None):
        ...
        for name in self.switches_openflow_names:
            ...
            output = subprocess.check_output(f"sudo ovs-ofctl dump-group-stats {name} --protocol=Openflow13", shell=True).decode('utf-8')  # Decode bytes to string for Python 3
...
def exists_bridge(name):
    try:
        subprocess.check_output(f"sudo ovs-vsctl br-exists {name}", shell=True)
        return True
